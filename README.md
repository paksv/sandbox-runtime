...
# Run with no network filtering at all, keeping the filesystem sandbox
# (macOS/Linux only — see network.unrestricted below)
srt --unrestricted-network npm test
```
...
- `network.unrestricted` - **Escape hatch:** disable network policy entirely (boolean, default: false). No proxy, SOCKS listener, Linux socat bridge or MITM CA is started, no proxy environment variables are set in the sandboxed process, and it talks to the network directly (macOS: the seatbelt profile emits `(allow network*)`; Linux: bwrap does not `--unshare-net`). Use it when a workload needs something the proxy cannot carry — UDP/QUIC, ICMP, raw sockets, mDNS, or a client that ignores `HTTP_PROXY` — but you still want the filesystem sandbox. `allowedDomains`, `deniedDomains`, `strictAllowlist` and `allowLocalBinding` are **ignored** (not an error, so a static policy can stay in place while the flag is flipped per run), while options that only exist inside the proxy are **rejected**: `tlsTerminate`, `mitmProxy`, `filterRequest`, `parentProxy`, `httpProxyPort`, `socksProxyPort`, and any `credentials` entry with `mode: "mask"` (`mode: "deny"` is fine). Filesystem, seccomp, PID/user-namespace and credential env-deny restrictions all still apply. **Not supported on Windows** — `initialize()` throws `WindowsSandboxError` with code `unrestricted_network_unsupported`, because the WFP egress fence is keyed on the `srt-sandbox` account machine-wide and permits only the loopback proxy ports, so skipping the proxy there would leave the process with no network at all. The network topology is fixed at `initialize()`: `updateConfig()` cannot flip this flag (it warns) — call `reset()` then `initialize()` instead. See Security Limitations for what you give up.
...
- Unrestricted Network (macOS/Linux): The `network.unrestricted` option turns the network layer off completely. There is no egress allowlist, no request audit or violation reporting for network access, no `filterRequest` callback, no TLS termination, and no credential injection or masking — masked credentials are rejected outright rather than silently handed to the process as an unusable sentinel. On Linux the process additionally **shares the host network namespace**, so it can reach services listening on the host's loopback interface (a local database, another SRT proxy, an internal dev server) that `--unshare-net` would otherwise have hidden. Filesystem, seccomp, PID/user-namespace and credential env-deny restrictions are unaffected, but treat a process running with this option as having the same network reach as an unsandboxed one. Not available on Windows, where `initialize()` rejects it rather than silently producing a process with no network at all.
